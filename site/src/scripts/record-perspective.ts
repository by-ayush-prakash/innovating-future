...
export const node = <K extends keyof HTMLElementTagNameMap>(tag: K, text = '', className = '') => {
...
let sequence = 0;
export function createPerspective(story: Perspective, onMode: (mode: ContextMode) => void) {
  ...
  profile.tabIndex = 0;
  profile.setAttribute('aria-label', `Perspective by ${story.source.speaker}. Focus to show more information.`);
  const identity = node('div', '', 'reading-attribution-person');
  const follow = node('button', 'See more in Follow →', 'reading-follow-person');
  follow.type = 'button';
  follow.setAttribute('aria-label', `See all of ${story.source.speaker}’s contributions in Follow`);
  follow.addEventListener('click', () => {
    element.dispatchEvent(new CustomEvent('record:follow-person', {
      bubbles: true,
      detail: story.source.speaker,
    }));
  });
  const attributionLine = node('p', '', 'reading-attribution-line');
  attributionLine.append(node('span', 'Perspective by: '), node('strong', story.source.speaker));
  identity.append(attributionLine);
  profile.append(identity);
  const contribution = node('div', '', 'reading-attribution-detail');
  contribution.append(node('p', story.source.role || 'Contributor to the Record', 'reading-attribution-role'));
  if (story.source.contribution) contribution.append(node('p', story.source.contribution, 'reading-attribution-focus'));
  ...
  reading.hidden = true;
  const transcript = node('section', '', 'source-conversation branch-transcript');
  ...
  for (const turn of sourceTurns) {
    ...
    header.append(node('strong', turn.speaker), node('span', turn.timestamp, 'source-turn-time'));
  ...
  const loadTurns = () => {
    ...
    if (!story.source.turnsUrl) { renderTurns(story.source.turns); return Promise.resolve(); }
    turns.setAttribute('aria-busy', 'true');
    turns.replaceChildren(node('p', 'Loading the source passage…'));
    sourceRequest = fetch(story.source.turnsUrl).then(response => {
      if (!response.ok) throw new Error('Source unavailable');
      return response.json();
    }).then(renderTurns).catch(() => {
      sourceRequest = null;
      const retry = node('button', 'Try loading the source again');
      retry.type = 'button';
      retry.addEventListener('click', () => { void loadTurns(); });
      turns.replaceChildren(node('p', 'The source passage could not load.'), retry);
  ...
  const setMode = (next: ContextMode, notify = true) => {
    ...
    if (next === 'listen') {
      ...
      const recordingFooter = node('footer', '', 'reading-source-foot');
      const readAlong = node('button', 'Read the transcript →');
      readAlong.type = 'button';
      readAlong.addEventListener('click', () => setMode('read'));
      recordingFooter.append(readAlong);
      recording.append(recordingHeader, media, recordingFooter);
    }
