type OpenChooser = { host: HTMLElement; close: () => void };
const openChoosers = new Set<OpenChooser>();
...
document.addEventListener(
  ...
  (event) => {
    ...
    for (const chooser of openChoosers)
      if (!chooser.host.contains(event.target as Node)) chooser.close();
  },
  true,
);
/** Shared context chooser for the opening source and every appended perspective. */
export function bindContextChooser(
  host: HTMLElement,
  trigger: HTMLElement,
  options: HTMLElement,
) {
  ...
  const open = () => {
    ...
    const above =
      innerHeight - bounds.bottom < height + 20 && bounds.top > height + 20;
  ...
  host.addEventListener("pointerleave", (event) => {
    if (event.pointerType === "mouse")
      leaveTimer = setTimeout(() => {
        if (!keyboardInteraction || !host.contains(document.activeElement))
          close();
      }, 180);
  });
